    ...
    cooperantesContainers.forEach(container => {
      ...
      participants.forEach(participant => {
        ...
        if (participantName) { // Solo agregar si el nombre no está vacío
          ...
          label.innerHTML = `
            <input type="checkbox" class="form-check-input" value="${participantName}">
            ${participantName}
          `;
    ...
    resultsContent.innerHTML = `
      ...
      <ul>
        ${participants.map(participant => `
          <li><strong>${participant}:</strong> $${participantCosts[participant].toFixed(2)}</li>
        `).join("")}
      </ul>
    `;
